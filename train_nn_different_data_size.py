...
def train_nn_with_different_nodes(training_data,nodes,  cuttoff=None):
    ...
    valid_size = int(training_data/3)
    test_size = int(training_data/3)
    Total_size = train_size + valid_size + test_size
    ######################################################################################
    # Load the input paramters
    ######################################################################################
    inputs = torch.tensor(np.load(directory_path+ "/input.npy"),dtype=dtype, device=device)[:Total_size]
    print(inputs.shape)
    
    # Load the state variable
    u = torch.tensor(np.load(directory_path+ "/u.npy"),dtype=dtype, device=device)[:Total_size]
    ...
    J = torch.tensor(np.load(directory_path+ "/Jacobian.npy"),dtype=dtype, device=device)[:Total_size].float()
    print(u.shape, J.shape)
...
def main():
    ...
    for training_data in Training_data:
        L2_full, L2_Jacobain, H1 = train_nn_with_different_nodes(training_data,nodes, cuttoff=100)
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(torch.log2(torch.tensor(Training_data)), L2_data, label="L2 accuracies", color="green")
    plt.plot(torch.log2(torch.tensor(Training_data)), L2_Jabobian_data, label="L2 Jacobian accuracie", color="blue")
    plt.plot(torch.log2(torch.tensor(Training_data)), H1_data, label="F2 accuracie", color="red")
