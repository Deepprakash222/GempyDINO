...
def train_nn_with_different_nodes(nodes, training_data,  cuttoff=None):
    ...
    print(device)
    ######################################################################################
    # Load the data(Input parameter, state variable u, and Jacobian Du)
    ######################################################################################
    ...
    for i in range(N):
        u_data = u[i].reshape(-1,1)
        result_sum += u_data @ u_data.T 
...
def main():
    ...
    for training_data in Training_data:
        L2_full, L2_Jacobain, H1 = train_nn_with_different_nodes(nodes,training_data,cuttoff=100)
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(Training_data, L2_data, label="L2 accuracies", color="green")
    plt.plot(Training_data, L2_Jabobian_data, label="L2 Jacobian accuracie", color="blue")
    plt.plot(Training_data, H1_data, label="F2 accuracie", color="red")
